...
public class Arm implements PIDOutput {
	...
	public static void setArmPID(double setpoint, double pk, double ik,
			double dk) {
		pid.enable();
		pid.setPID(pk, ik, dk);
	...
	public static void adjustArmPID(double setpoint, double pk, double ik,
			double dk) {
		pid.enable();
		pid.setPID(pk, ik, dk);
	...
	public static void setArmPos(Constants.ARM_POS pos) {
		if (pos == Constants.ARM_POS.LOW) {
			setArmPID(Constants.ARM_LOW_POS, Constants.ARM_LOW_P,
					Constants.ARM_LOW_I, Constants.ARM_LOW_D);
		} else if (pos == Constants.ARM_POS.DEF) {
			setArmPID(Constants.ARM_DEF_POS, Constants.ARM_DEF_P,
					Constants.ARM_DEF_I, Constants.ARM_DEF_D);
		} else if (pos == Constants.ARM_POS.ING) {
			setArmPID(Constants.ARM_ING_POS, Constants.ARM_ING_P,
					Constants.ARM_ING_I, Constants.ARM_ING_D);
		} else if (pos == Constants.ARM_POS.SHOOT) {
			if (shootingAdjustment == 0) {
				setArmPID(Constants.ARM_SHOOT_POS, Constants.ARM_SHOOT_P,
						Constants.ARM_SHOOT_I, Constants.ARM_SHOOT_D);
			} else {
				adjustArmPID(Constants.ARM_SHOOT_POS,
						Constants.ARM_SHOOT_ADJUST_P,
						Constants.ARM_SHOOT_ADJUST_I,
						Constants.ARM_SHOOT_ADJUST_D);
			}
		} else if (pos == Constants.ARM_POS.AUTO_SHOOT) {
			setArmPID(Constants.ARM_AUTO_SHOOT_POS, Constants.ARM_AUTO_SHOOT_P,
					Constants.ARM_AUTO_SHOOT_I, Constants.ARM_AUTO_SHOOT_D);
		}
	}
	...
	public static void setSpeedFast(double value) {
		pid.disable();
		if (value < 0) {
			setPower(-value);
		} else if (value > 0) {
